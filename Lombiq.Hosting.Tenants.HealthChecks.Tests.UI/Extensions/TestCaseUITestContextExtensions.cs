...
public static partial class TestCaseUITestContextExtensions
{
    ...
    private static void HealthCheckShouldBe(this UITestContext context, HealthStatus status) =>
        context.Get(By.TagName("pre")).GetTextTrimmed().ShouldBe(status.ToString());

    private static void AssertUnhealthyTenants(this UITestContext context, params string[] expected) =>
        context
            .GetAllWhenOneExists(By.CssSelector(".ta-content > ul > li"))
            .Select(element => WhitespaceRegex.Replace(element.Text.Trim(), " "))
            .ToArray()
            .ShouldBe(expected);
}
